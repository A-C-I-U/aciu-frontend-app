...
export default function BranchInfoCard({
  ...
  const { createdAt, meetingLocation, branchKey, branchName, branchChairman, branchChairmanEmail } =
    branchInfo;
    ...
    <div className="bg-white rounded-2xs py-6 px-5 flex flex-col gap-4 lg:gap-6">
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-0 justify-between lg:items-center">
        <div className="flex  items-center gap-4">
          ...
          <h5 className="text-xl text-aciu-border-grey">{branchName}</h5>
        </div>
        <button
          onClick={onEditBranch}
          className="text-aciu-abriba border border-aciu-abriba hover:bg-aciu-bg-grey rounded-xl flex items-center gap-2 max-w-fit text-sm p-3 lg:text-base lg:p-4 shadow-[0px_1px_3px_0px_#0D0D120D]"
        >
      ...
      <div className="hidden lg:grid lg:grid-cols-2 ml:grid-cols-[1fr_.5fr_.5fr_.5fr] items-start gap-4">
        {branchInfoDetails.map(({ title, content }, index) => (
          <div key={index} className="flex flex-col gap-2 max-w-83.25">
            <p className="text-aciu-abriba">{title}</p>
            <p className="font-medium text-aciu-border-grey">{content}</p>
          </div>
        ))}
      </div>
    </div>

...
export default function SignUpPage() {
    ...
    if (success) {
        if (showAdminList) {
            return <AdminList onBack={() => setShowAdminList(false)} userEmail={values?.email} />;
        }
        if (showUserDetails) {
            return <UserDetails values={values} onBack={() => setShowUserDetails(false)} />;
        }
        return (
            <SuccessPrompt
                onContactAdmin={() => setShowAdminList(true)}
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                    ...
                                    <Button
                                        ...
                                        onClick={(e) => {
                                            if (step === 2) {
                                                e.preventDefault();
                                                stepContent[step].submit(formikValues, userId);
                                            }
                                        }}
                                    >

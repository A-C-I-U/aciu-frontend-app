      ...
      <>
        {step === 2 ? (
          ...
          <div className="resources-modal-section flex flex-col h-full overflow-hidden">
            <ShellHeader title="Nominate a Project" onClose={onClose} />
            <NominateProjectForm
  ...
  const handleSubmit = async (values: ProjectFormValues, actions: any) => {
    try {
      ...
      const payload = {
        ...
        estimatedCostUSD: values.cost ? values.cost.toString() : "",
      ...
      enqueueSnackbar(error.response?.data?.message || error.message || 'Failed to nominate project', {
        variant: 'error',
      });
      ...
      {({ values, setFieldValue, isSubmitting, errors }) => {
        return (
          <Form className="flex flex-col h-full overflow-hidden">
            <div className="resources-modal-body flex-1 overflow-y-auto px-5.5 py-4">
                  ...
                  >
                    {!values.image ? (
                      <div className="gap-2 flex flex-col">
                        ...
                        relative h-53.5 rounded-[.625rem] w-full 
                        ${isSubmitting ? 'bg-grayscale-100' : 'bg-[#F9FFF8]'}
                        ${errors.image ? 'border-error-normal' : 'border-[#00B887]'}`}
                        >
                          <div
                            className="items-center justify-center
                            flex flex-col gap-3 w-full"
                          >
                            <CloudIcon color="#00B887" />
                            <p className="text-[#3E3E3E] font-montserrat font-medium text-sm">
            ...
            <div className="px-5.5 py-6 flex items-center gap-2 flex-shrink-0">
              <button
                type="submit"
                className="btn w-full !rounded-full !py-4 !bg-[#00B887] !text-white !font-montserrat !font-semibold !text-base !normal-case"
                disabled={isSubmitting}
              >
